...
func WebUI(api http.Handler, directory string) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Security-Policy", "default-src 'self'; script-src 'self' blob:; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data: blob:; frame-src 'self' blob:; connect-src 'self' blob: https://en.wiktionary.org; worker-src 'self' blob:; object-src 'none'; base-uri 'self'; frame-ancestors 'none'")
