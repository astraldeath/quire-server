...
var webRoute = regexp.MustCompile(`^/(library|reading|account|settings(/(appearance|library|backups|statistics))?|admin(/(overview|libraries|folders|settings|accounts|invites|backups))?|books/[a-f0-9]{64}(/(read|tracking))?|series/[^/]+(/tracking)?)$`)
