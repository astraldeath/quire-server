...
func TestPrivacyMigrationFromVersionNine(t *testing.T) {
	...
	if _, err = s.db.Exec("DROP TABLE folder_catalog; DROP TABLE privacy_settings; ALTER TABLE tracking_links DROP COLUMN is_private; ALTER TABLE scan_status DROP COLUMN skipped_files; ALTER TABLE scan_status DROP COLUMN omitted_skipped_files; DROP TABLE catalog_sources; DROP TABLE catalog_operations; DROP TABLE opds_passwords; PRAGMA user_version=9;"); err != nil {
		t.Fatal(err)
	}
	s.Close()
	s, err = Open(path)
