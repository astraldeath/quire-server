...
func TestWebDeepLinksAndPrivatePaths(t *testing.T) {
	...
	for _, path := range []string{"/library", "/reading", "/series/A%2FB/tracking", "/books/" + strings.Repeat("a", 64) + "/read", "/settings/backups", "/settings/statistics", "/settings/privacy", "/settings/updates", "/admin/accounts", "/account"} {
